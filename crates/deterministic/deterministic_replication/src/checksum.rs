...
impl ChecksumSendPlugin {
    ...
    /// `current_tick - max_rollback_ticks - 1`. A rollback may target a
    /// tick `r >= current - max_rollback_ticks`; it re-stamps the restore
    /// value AT `r` and replays `r+1..`, so `current - max_rollback - 1`
    /// is the newest tick no rollback can ever rewrite — every report is
    /// final and every logged mismatch is a real committed divergence.
    /// Reporting `LastConfirmedInput.tick` directly (as before) sends
    /// hashes that a repairing rollback can still invalidate — the stale
    /// report reads as a mismatch even though both peers' states agree,
    /// scattering comparison noise across the run. The tick must also be
    /// confirmed (`<= LastConfirmedInput.tick`): hashing an unconfirmed
    /// tick would report prediction garbage. `PredictionHistory` resolves
    /// at-or-before, so the older read is exact. (0.29 note: upstream
    /// prunes history at the rollback floor — #1626 — so the fork prunes
    /// one tick deeper to keep this report tick resolvable.)
    ///
    ...
    fn compute_and_send_checksum(
        ...
        // The newest tick no future rollback can still rewrite. A rollback
        // to tick `r` both re-stamps the restore value AT `r` and replays
        // `r+1..`, and `do_rollback` bounds `current - r <= max_rollback`,
        // so `current - max_rollback - 1` is the newest tick that is final
        // in both senses. History is pruned one tick deeper than the
        // rollback floor (see `update_prediction_history`), so this tick is
        // still buffered. Guard the subtraction explicitly: Tick arithmetic
            ...
            checksum_archetype
                ...
                .for_each(|(component_id, storage_type)| {
                        ...
                        world.state.confirmed_fallback_fns.get(component_id)
                    {
                        ...
                        // Entities in this archetype may not carry the
                        // confirmed history at all (no delivery write ever
                        // landed) — match the server, which only hashes
                        // entities that actually hold the component.
                        if archetype.contains(*confirmed_id) {
